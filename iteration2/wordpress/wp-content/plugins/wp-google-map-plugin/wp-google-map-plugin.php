	...
	class FC_Google_Maps_Lite
	{
		...
		function wpgmp_frontend_scripts() {
			...
			if ( get_option( 'wpgmp_api_key' ) != '' ) {
				$google_map_api = 'https://maps.google.com/maps/api/js?key='.get_option( 'wpgmp_api_key' ).'&callback=initwpmaps&libraries=geometry,places,drawing&language='.$language;
			} else {
				$google_map_api = 'https://maps.google.com/maps/api/js?&callback=initwpmaps&libraries=geometry,places,drawing&language='.$language;
			}
			...
			if ( $scripts ) {
				foreach ( $scripts as $script ) {
					wp_enqueue_script( $script['handle'], $script['src'], $script['deps'], false, $where );
				}
			}
		...
		function wpgmp_backend_scripts() {
			...
			if ( get_option( 'wpgmp_api_key' ) != '' ) {
				$google_map_api = 'https://maps.google.com/maps/api/js?key='.get_option( 'wpgmp_api_key' ).'&callback=initwpmaps&libraries=geometry,places,drawing&language=en';
			} else {
				$google_map_api = 'https://maps.google.com/maps/api/js?&callback=initwpmaps&libraries=geometry,places,drawing&language=en';
			}
